...
const VSTS_SPS_BASE_URL = "https://app.vssps.visualstudio.com";
...
export class ExtensionInstall extends extBase.ExtensionBase<ExtensionInstallResult> {
    protected description = "Install a Visual Studio Services Extension in a list of VSTS Accounts.";
    ...
    private _resolveInstallationAccounts(galleryApi: gallerym.IQGalleryApi, accounts: string[]): Q.Promise<Account[]> {
        if (!accounts || accounts.length === 0) {
            return Q.resolve([]);
        }

        const result: Q.Deferred<Account[]> = Q.defer<Account[]>();
        ...
        galleryApi.connect()
            ...
            .then((userAccounts: Account[]) => {
                ...
                return result.resolve(accounts.map(accountName => { return { accountId: getAccountId(accountName), accountName: accountName }; }));
            })
            .fail(err => {
                result.reject(err);
            });
        ...
        return galleryApi.requestAcquisition(<GalleryInterfaces.ExtensionAcquisitionRequest>{
            ...
            .fail((err) => {
                ...
                if (this._isAlreadyInstalledError(err)) {
                    trace.debug(`Succeeded GalleryApi.requestAcquisition in ${accountName}: Alreeady installed`);
    ...
    private _getAccountsByMemberId(userId: string): Q.Promise<Account[]> {
        const accountsBaseUrl = this._getAccountsServiceBaseUrl(this.webApi.serverUrl);
        const accountsApi = new AccountsApi(accountsBaseUrl, [this.webApi.authHandler]);
        return accountsApi.getAccountsByMemberId(userId);
    }


    /**
     * Determine the Accounts URL based on the service URL.
     * TODO: Check if it is possible to do it using the REST API.
     *
     * @param  {string} serviceUrl
     * @returns string
     */
    private _getAccountsServiceBaseUrl(serviceUrl: string): string {
        // When hosted, return SPS URL
        if (serviceUrl.toLowerCase().indexOf(".visualstudio.com") > 0) {
            return VSTS_SPS_BASE_URL;
        }

        // Otherwise return the original serviceUrl
        return serviceUrl;
    }
}
...
/**
 * TODO: Remove this as soon as it is included in the vso-node-api module
 *
 */
...
class AccountsApi extends ClientApiBase {
    ...
    public getAccountsByMemberId(memberId: string): Q.Promise<Account[]> {
        ...
        this.vsoClient.getVersioningData("2.0", "account", "229a6a53-b428-4ffb-a835-e8f36b5b4b1e", null, { memberId: memberId })
            .then(versioningData => {
