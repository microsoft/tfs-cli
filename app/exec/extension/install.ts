	...
	public exec(): Promise<ExtensionInstallResult> {
		...
		return this._getExtensionInfo()
			.then(extInfo => {
				...
				return this.commandArgs.accounts.val().then(accounts => {
					...
					const installations = [...accounts].map((account): Promise<[string, EmsInterfaces.InstalledExtension]> => {
						...
						return emsApi.installExtensionByName(extInfo.publisher, extInfo.id).then(installation => [account, installation]);
					});
