...
export abstract class ManifestBuilder {
	...
	public producesMetadata: boolean = false;
		...
		return [
			{
				...
				content: JSON.stringify(
					this._getLocResult(this.expandResourceFile(translations), this.expandResourceFile(defaults)),
					null,
					4,
				).replace(/\n/g, os.EOL),
			},
	 ...
	 */
	protected singleValueProperty(
		path: string,
		value: any,
		manifestKey: string,
		override: boolean = false,
		duplicatesAreErrors: boolean = false,
	): boolean {
		...
		if (!override && existingValue !== undefined) {
			if (duplicatesAreErrors) {
				throw new Error(
					trace.format(
						"Multiple values found for '%s'. Ensure only one occurrence of '%s' exists across all manifest files.",
						manifestKey,
						manifestKey,
					),
				);
				...
				trace.warn(
					"Multiple values found for '%s'. Ignoring future occurrences and using the value '%s'.",
					manifestKey,
